...
class Room(db.Model):
    ...
    def remove_user(self, user):
        ...
        try:
            ...
            if user in dictUser:
                print
                "Deleting user " + str(user) + " from dic " + str(dictUser)
                del dictUser[user]
                self.userStr = str(dictUser)
...
class ConnectPage(webapp2.RequestHandler):
    def post(self):
        ...
        print
        "ConnectPage: key-" + str(key) + ", user-" + str(user) + ", room-" + str(room_key)
        with LOCK:
...
class DisconnectPage(webapp2.RequestHandler):
    def post(self):
        ...
        room_key, user = key.split('/')
        print
        "DisconnectPage: key-" + str(key) + ", user-" + str(user) + ", room-" + str(room_key)
        with LOCK:
            room = Room.get_by_key_name(room_key)
            if room and room.has_user(user):
                other_user = room.get_other_user(user)
...
class MessagePage(webapp2.RequestHandler):
    def post(self):
        ...
        print
        "MessagePage: message-" + str(message) + ", user-" + str(user) + ", room-" + str(room_key)
        with LOCK:
            room = Room.get_by_key_name(room_key)
            if room:
